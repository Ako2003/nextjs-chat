...
export default function Chat() {
    ...
    const sendMessage = (value) => {
        ...
        setMessages([
            ...messages,
            {id: messages.length + 1, text: value, user: 'Akif Mursalov', src: 'https://i.pravatar.cc/150?u=a04258a2462d826712d', time: currentTime()}
        ])
        setValue('')
    }
  ...
  return (
    <section ref={chatRef} className='grid bg--400 relative max-w-full'>
        {messages.map((message, index) => (
            <div className={`mx-40 my-2 gap-2 max-w-screen-sm ${message.id % 2 ? "justify-self-start" : "justify-self-end"} ${message.id === 1 && 'mt-32'} ${message.id === messages.length && 'mb-32'}`} key={index}>
                {message.id % 2 ? 
